...
class Menu
  GRID = 20
  def display_stuff
    text1 = Text.new("Welcome to the snake game!", x: (Window.width/2)-350, y:20, size: 55)
    Text.new("You can control the snake using arrows or WSAD keys.", x: (Window.width/2)-290, y:100, size: 25)
    Text.new("You can score points by eating apples.", x: (Window.width/2)-210, y:130, size: 25)
    Text.new("You lose when snake bits his tail.", x: (Window.width/2)-190, y:160, size: 25)
    Text.new("Press Spacebar to start the game.", x: (Window.width/2)-190, y:220, size: 25)
    Square.new(x: 30 * GRID, y: 20 * GRID, size: GRID - 1, color: 'white') #displays the snake in the menu window
    ...
    Square.new(x: 23 * GRID, y: 20 * GRID, size: GRID - 1, color: '#b7b0b0') #displays the apple
  end
